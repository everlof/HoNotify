...
def srp_auth(login,password):
    ...
    usr = User( login, None, hash_alg=SHA256, ng_type=NG_CUSTOM, n_hex = s2_n , g_hex = s2_g )
    ...
    B = res['B'].decode('hex')
    salt2 = res['salt2']
    usr.password = sha256(md5(password + salt2 + '[!~esTo0}').hexdigest() + 'taquzaph_?98phab&junaj=z=kuChusu').hexdigest()
